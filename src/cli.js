...
import { statsValidate, stats } from './utils/util.js';
...
if (argv.length === 1) {
  mdLinks(argv[0], options).then(arr => arr.forEach(obj => console.log(`File: ${obj.file}\nLink: ${obj.href}\nText: ${obj.text}\n`)));
}
...
if (argv.length === 2) {
  if (argv[1] === '--validate') {
    ...
    options.stats = true;
    mdLinks(argv[0], options).then(arr => console.log(`Total : ${stats(arr).total}\nUnique : ${stats(arr).unique}`));
  }
}
